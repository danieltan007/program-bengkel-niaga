...
$(document).ready(function(){
     ...
     $(document).on("change","input[id^='jml-']", function(e){
          var jml = $(this).val();
          var kode = $(this).attr('id').substr(4,11);
          $.ajax({
               type: "post",
               url: "do_mekanikjmlbrg.php",
               data: {jml:jml, kode:kode},
               success: function(){
                    refresh();
               }
          });
     ...
     $(document).on("change","input[id^='disc-']", function(e){
          var diskon = $(this).val();
          var kode = $(this).attr('id').substr(5,12);
          $.ajax({
               type: "post",
               url: "do_mekanikdiscbrg.php",
               data: {diskon:diskon, kode:kode},
               success: function(){
                    refresh();
               }
          });
     ...
     $(document).on("change","input[id^='kort-']", function(){
          var kode3 = $(this).attr("id").substr(5,12);
          var korting = $(this).val();
          $.ajax({
               type: 'post',
               url: "do_mekanikkortbrg.php",
               data: {kode3:kode3, korting:korting},
               success: function(){
                    refresh();
               }
          }); 
     ...
     $(document).on("change","input[id^='ongkos-']", function(){
          var kode = $(this).attr('id').substr(7,8);
          var ongkos = $(this).val();

          $.ajax({
               url: "do_mekanikupdateongkos.php",
               type: "post",
               data: {kode:kode, ongkos:ongkos},
               success: function()
               {
                    update();
                    totalharga();
