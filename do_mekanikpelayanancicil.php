...
<?php  }
     ...
     <?php
          ...
          {    
               ...
               {
                    $sql3 = "select * from `tabel barang` where kd_brg = '$data[kd_brg]' ";
                    ...
                    ('$kodetrans2', '$data[nm_brg]', '$data[merek]', '$data[jml_brg]', '$data[diskon]', '$data[total]', '$tgl', 'cicil', '$data[hrg_brg]', '$data[kd_brg]', '$data[korting]')";
                    mysqli_query($conn, $sql5);

                    //hitung jumlah keuntungan per barang
                    $sql7 = "select * from `tabel barang pusat` where kd_brg = '$data[kd_brg]' ";
                    $cari = mysqli_query($conn, $sql7);
                    $data7 = mysqli_fetch_array($cari);
                    $modal = (int)$data7['hrg_modal'];
                    ...
                    $brgstok = (int)$data2['sto_toko'];
                    $sisastok = $brgstok - $brgbeli;

                    $sql4 = "update `tabel barang` set sto_toko = '$sisastok' where kd_brg = '$data[kd_brg]'";
               ...
               else if ($data['sumber'] == 'toko lain')
               {
                    ...
                    $sql5 = "insert into `detail transaksi` 
                    (id_trans, nm_brg, mrk_brg, jml_beli, diskon, total_harga, tgl_trns, status, hrg_brg, kd_brg, korting) values 
                    ('$kodetrans2', '$data[nm_brg]', '', '$data[jml_brg]', '0', '$data[total]', '$tgl', 'cicil', '$data[hrg_brg]', '$data[kd_brg]', '$data[korting]')";
                    mysqli_query($conn, $sql5);
