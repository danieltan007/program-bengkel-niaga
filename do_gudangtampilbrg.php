<?php 
	...
	if($merek == "" && $sup == "")
     ...
     {
          $sql = "select * from `tabel barang` where nm_brg like '%$cari%' and mrk_brg = '$merek' ";
          $search = mysqli_query($conn, $sql);
     }
     else if($merek == "" && $sup != "")
     {
          $sql = "select * from `tabel barang` where nm_brg like '%$cari%' and supp_brg = '$sup' ";
          $search = mysqli_query($conn, $sql);
     }
     else
     {
          $sql = "select * from `tabel barang` where nm_brg like '%$cari%' and mrk_brg = '$merek' and supp_brg = '$sup' ";
          $search = mysqli_query($conn, $sql);
	}
?>
 ...
 while ($data = mysqli_fetch_array($search)) 
  { 
    ...
    <tr id="<?php echo $data['kd_brg']; ?>">
		...
		<td data-target="supp"><?php echo $data['supp_brg']; ?></td>
		<td data-target="stok"><?php echo $data['sto_brg']; ?></td>
		<td data-target="hrg"><?php echo $data['hrg_brg']; ?></td>
		<td><?php echo $edit; ?></td>
